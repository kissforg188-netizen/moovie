...
<body>
  ...
  <main class="wrap">
    ...
    <?php elseif ($page === 'automation'): ?>
      ...
      <section class="card fade-up">
        <h2>Hashtag Fit Lab</h2>
        <p class="muted"><?= h($hashtagFitLab['summary']) ?></p>
        <p class="muted">เกรด <?= h($hashtagFitLab['grade']) ?> · <?= (int)$hashtagFitLab['score'] ?>/100 · หน้าต่าง <?= (int)$hashtagFitLab['windowDays'] ?> วัน</p>
        <p><?= h($hashtagFitLab['mixTip']) ?></p>
        <div class="stats" style="margin-top:0.75rem">
          <article><p>มีเมตริก</p><strong><?= (int)$hashtagFitLab['counts']['postsWithMetrics'] ?></strong></article>
          <article><p>สไตล์มีข้อมูล</p><strong><?= (int)$hashtagFitLab['counts']['bandsWithData'] ?></strong></article>
          <article><p>ร้อน</p><strong><?= (int)$hashtagFitLab['counts']['hot'] ?></strong></article>
          <article><p>คำแนะนำ</p><strong><?= (int)$hashtagFitLab['counts']['suggestions'] ?></strong></article>
        </div>
        <?php
          $tagBandRows = array_values(array_filter($hashtagFitLab['bands'], fn($b) => ($b['samples'] ?? 0) > 0));
          if (!$tagBandRows): ?>
          <p class="muted">ยังไม่มีเมตริกรายสไตล์แฮชแท็ก — โพสต์มือแล้วกรอกผลที่ Results</p>
        <?php else: ?>
          <ol>
            <?php foreach (array_slice($tagBandRows, 0, 4) as $b): ?>
              <li>
                <strong>[<?= h($b['status'] === 'hot' ? 'ร้อน' : ($b['status'] === 'cold' ? 'เย็น' : ($b['status'] === 'steady' ? 'นิ่ง' : 'ยังไม่มีข้อมูล'))) ?>]</strong>
                <?= h($b['bandLabel']) ?>
                <div class="muted">คะแนน <?= (int)$b['score'] ?>/100 · n=<?= (int)$b['samples'] ?> · CTR ~<?= h((string)round($b['avgCtr'] * 100, 1)) ?>% · แท็ก avg ~<?= h((string)$b['avgTagCount']) ?></div>
                <div class="muted"><?= h($b['tip']) ?></div>
              </li>
            <?php endforeach; ?>
          </ol>
        <?php endif; ?>
        <?php if ($hashtagFitLab['suggestions']): ?>
          <p class="muted" style="margin-top:0.75rem"><strong>คำแนะนำคิววันนี้ (ไม่เปลี่ยนอัตโนมัติ)</strong></p>
          <ul>
            <?php foreach (array_slice($hashtagFitLab['suggestions'], 0, 4) as $s): ?>
              <li><?= h($s['productName']) ?>: <?= h($s['currentLabel']) ?> → <?= h($s['suggestedLabel']) ?> — <?= h($s['reason']) ?></li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
        <ul>
          <?php foreach (array_slice($hashtagFitLab['actions'], 0, 3) as $a): ?>
            <li><strong><?= h($a['title']) ?></strong> — <?= h($a['detail']) ?></li>
          <?php endforeach; ?>
        </ul>
        <div class="actions">
      ...
      <section class="card fade-up">
        <h2>Script Fit Lab</h2>
        <p class="muted"><?= h($scriptFitLab['summary']) ?></p>
        <p class="muted">เกรด <?= h($scriptFitLab['grade']) ?> · <?= (int)$scriptFitLab['score'] ?>/100 · หน้าต่าง <?= (int)$scriptFitLab['windowDays'] ?> วัน</p>
        <p><?= h($scriptFitLab['mixTip']) ?></p>
        <div class="stats" style="margin-top:0.75rem">
          <article><p>มีเมตริก</p><strong><?= (int)$scriptFitLab['counts']['postsWithMetrics'] ?></strong></article>
          <article><p>โครงมีข้อมูล</p><strong><?= (int)$scriptFitLab['counts']['bandsWithData'] ?></strong></article>
          <article><p>ร้อน</p><strong><?= (int)$scriptFitLab['counts']['hot'] ?></strong></article>
          <article><p>flat</p><strong><?= (int)$scriptFitLab['counts']['flatSamples'] ?></strong></article>
        </div>
        <?php
          $scriptBandRows = array_values(array_filter($scriptFitLab['bands'], fn($b) => ($b['samples'] ?? 0) > 0));
          if (!$scriptBandRows): ?>
          <p class="muted">ยังไม่มีเมตริกรายโครงสคริปต์ — โพสต์มือแล้วกรอกผลที่ Results</p>
        <?php else: ?>
          <ol>
            <?php foreach (array_slice($scriptBandRows, 0, 4) as $b): ?>
              <li>
                <strong>[<?= h($b['status'] === 'hot' ? 'ร้อน' : ($b['status'] === 'cold' ? 'เย็น' : ($b['status'] === 'steady' ? 'นิ่ง' : 'ยังไม่มีข้อมูล'))) ?>]</strong>
                <?= h($b['bandLabel']) ?>
                <div class="muted">คะแนน <?= (int)$b['score'] ?>/100 · n=<?= (int)$b['samples'] ?> · CTR ~<?= h((string)round($b['avgCtr'] * 100, 1)) ?>%</div>
        ...
        <?php endif; ?>
        <?php if ($scriptFitLab['suggestions']): ?>
          <p class="muted" style="margin-top:0.75rem"><strong>คำแนะนำคิววันนี้ (ไม่เปลี่ยนอัตโนมัติ)</strong></p>
          <ul>
            <?php foreach (array_slice($scriptFitLab['suggestions'], 0, 4) as $s): ?>
              <li><?= h($s['productName']) ?>: <?= h($s['currentLabel']) ?> → <?= h($s['suggestedLabel']) ?> — <?= h($s['reason']) ?></li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
        <ul>
          <?php foreach (array_slice($scriptFitLab['actions'], 0, 3) as $a): ?>
            <li><strong><?= h($a['title']) ?></strong> — <?= h($a['detail']) ?></li>
          <?php endforeach; ?>
        </ul>
        <div class="actions">
      ...
      <section class="card fade-up">
        <h2>Offer Fit Lab</h2>
        <p class="muted"><?= h($offerFitLab['summary']) ?></p>
        <p class="muted">เกรด <?= h($offerFitLab['grade']) ?> · <?= (int)$offerFitLab['score'] ?>/100 · หน้าต่าง <?= (int)$offerFitLab['windowDays'] ?> วัน</p>
        <p><?= h($offerFitLab['mixTip']) ?></p>
        <div class="stats" style="margin-top:0.75rem">
          <article><p>มีเมตริก</p><strong><?= (int)$offerFitLab['counts']['postsWithMetrics'] ?></strong></article>
          <article><p>มุมมีข้อมูล</p><strong><?= (int)$offerFitLab['counts']['bandsWithData'] ?></strong></article>
          <article><p>ร้อน</p><strong><?= (int)$offerFitLab['counts']['hot'] ?></strong></article>
          <article><p>ขายแข็ง</p><strong><?= (int)$offerFitLab['counts']['hardPushSamples'] ?></strong></article>
          <article><p>none</p><strong><?= (int)$offerFitLab['counts']['noneSamples'] ?></strong></article>
        </div>
        <?php
          $offerBandRows = array_values(array_filter($offerFitLab['bands'], fn($b) => ($b['samples'] ?? 0) > 0));
          if (!$offerBandRows): ?>
          <p class="muted">ยังไม่มีเมตริกรายมุมเสนอ — โพสต์มือแล้วกรอกผลที่ Results</p>
        <?php else: ?>
          <ol>
            <?php foreach (array_slice($offerBandRows, 0, 4) as $b): ?>
              <li>
                <strong>[<?= h($b['status'] === 'hot' ? 'ร้อน' : ($b['status'] === 'cold' ? 'เย็น' : ($b['status'] === 'steady' ? 'นิ่ง' : 'ยังไม่มีข้อมูล'))) ?>]</strong>
                <?= h($b['bandLabel']) ?>
                <div class="muted">คะแนน <?= (int)$b['score'] ?>/100 · n=<?= (int)$b['samples'] ?> · CTR ~<?= h((string)round($b['avgCtr'] * 100, 1)) ?>%</div>
        ...
        <?php endif; ?>
        <?php if ($offerFitLab['suggestions']): ?>
          <p class="muted" style="margin-top:0.75rem"><strong>คำแนะนำคิววันนี้ (ไม่เปลี่ยนอัตโนมัติ)</strong></p>
          <ul>
            <?php foreach (array_slice($offerFitLab['suggestions'], 0, 4) as $s): ?>
              <li><?= h($s['productName']) ?>: <?= h($s['currentLabel']) ?> → <?= h($s['suggestedLabel']) ?> — <?= h($s['reason']) ?></li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
        <ul>
          <?php foreach (array_slice($offerFitLab['actions'], 0, 3) as $a): ?>
            <li><strong><?= h($a['title']) ?></strong> — <?= h($a['detail']) ?></li>
          <?php endforeach; ?>
        </ul>
        <div class="actions">
        ...
        <p class="note"><?= h($offerFitLab['disclaimer']) ?></p>
      </section>
