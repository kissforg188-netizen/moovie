...
try {
    ...
    if ($method === 'GET' && $action === 'export') {
        ...
        if ($format === 'md' || $format === 'markdown') {
            ...
            if (in_array($scope, ['intake', 'results-intake', 'metrics-queue'], true)) {
                $intake = build_results_intake(today_iso());
                header('Content-Disposition: attachment; filename="affiliate-results-intake-'.today_iso().'.md"');
                echo results_intake_to_markdown($intake);
                automation_log_finish($job, 'success', 'exported md results-intake');
                exit;
            }
            if (in_array($scope, ['creative', 'creative-performance', 'hooks'], true)) {
                $creative = build_creative_performance(today_iso());
                header('Content-Disposition: attachment; filename="affiliate-creative-performance-'.today_iso().'.md"');
                echo creative_performance_to_markdown($creative);
                automation_log_finish($job, 'success', 'exported md creative-performance');
                exit;
            }
            if (in_array($scope, ['publish', 'publish-queue', 'manual-publish'], true)) {
                $publish = build_publish_queue(today_iso());
                header('Content-Disposition: attachment; filename="affiliate-publish-queue-'.today_iso().'.md"');
                echo publish_queue_to_markdown($publish);
            ...
            if (in_array($scope, ['seasonal-fit', 'seasonal', 'season', 'trend-season'], true)) {
                $lab = build_seasonal_fit_lab(today_iso());
                header('Content-Disposition: attachment; filename="affiliate-seasonal-fit-lab-'.today_iso().'.md"');
                echo seasonal_fit_lab_to_markdown($lab);
            ...
            if (in_array($scope, ['offer-fit', 'offer', 'offers', 'value-offer', 'value-fit'], true)) {
                $lab = build_offer_fit_lab(today_iso());
                header('Content-Disposition: attachment; filename="affiliate-offer-fit-lab-'.today_iso().'.md"');
                echo offer_fit_lab_to_markdown($lab);
