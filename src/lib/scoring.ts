...
function commissionRateScore(rate: number): number {
...
/**
 * Soft score from expected baht commission per sale (price × rate%).
 * A 50% cut on ฿20 is weaker than 12% on ฿299 for affiliate experiments.
 */
export function expectedCommissionBaht(price: number, rate: number): number {
  return Math.max(0, price) * Math.max(0, rate) / 100;
}

export function expectedCommissionScore(price: number, rate: number): number {
  const baht = expectedCommissionBaht(price, rate);
...
export function scoreProduct(
  ...
  const rateScore = commissionRateScore(product.commissionRate);
  const expected = expectedCommissionScore(
    product.price,
    product.commissionRate,
  );
  // Blend rate + expected baht so tiny high-% items don't dominate ranking
  const commission = rateScore * 0.65 + expected * 0.35;
  ...
  const learningBoost = learningRankBoost(product.id, learning);
  ...
  const total = base * 0.85 + historyBoost * 0.15 + learningBoost;
  ...
  return {
    ...
    learningBoost: Math.round(learningBoost * 10) / 10,
    total: Math.round(total * 10) / 10,
  };
}

/** Short Thai explanation of why a product ranks well (transparency, not a sales claim). */
export function explainScore(product: Product, score: ScoreBreakdown): string {
  const dims: { label: string; value: number }[] = [
    { label: "ค่าคอม", value: score.commission },
    { label: "ราคา impulse", value: score.impulsePrice },
    { label: "pain ชัด", value: score.painClarity },
    { label: "ถ่ายคลิปง่าย", value: score.videoEase },
    { label: "ซีซัน", value: score.seasonal },
  ];
  const top = [...dims].sort((a, b) => b.value - a.value).slice(0, 2);
  const baht = expectedCommissionBaht(product.price, product.commissionRate);
  const parts = [
    `จุดแข็ง: ${top.map((t) => t.label).join(" · ")}`,
    `ค่าคอมคาดหวัง ~฿${baht.toFixed(0)}/ชิ้น (ทดลองจากราคา×%)`,
  ];
  if (score.learningBoost > 0) {
    parts.push("ได้ soft boost จากผลเย็นก่อนหน้า");
  } else if (score.learningBoost < 0) {
    parts.push("ถูกลดน้ำหนักเล็กน้อยจากผลเย็นก่อนหน้า");
  }
  if (score.historyBoost > 60) {
    parts.push("ประวัติโพสต์ที่บันทึกดูโอเคกว่าค่ากลาง");
  }
  return parts.join(" — ");
}
