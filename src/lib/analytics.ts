...
export function analyzePosted(
  ...
  if (winners.length === 0) {
    ...
    const vanity = performances.filter((p) => {
      const views = p.post.metrics?.views ?? 0;
      const orders = p.post.metrics?.orders ?? 0;
      return views >= 500 && orders === 0;
    });
    if (vanity.length > 0) {
      recommendations.push(
        `${vanity.length} โพสต์วิวสูงแต่ยังไม่มียอดสั่ง — ลองมุม pain ชัดขึ้น หรือ CTA ที่ชวนเทียบสเปก ไม่เร่งซื้อ (ทดลอง)`,
      );
    }
  }
