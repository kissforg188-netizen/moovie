...
export async function runMorningWorkflow(date = todayISO()) {
  return updateDb((db) => {
    const ranked = rankProducts(db.products, 5);
    ...
    for (const item of ranked) {
      ...
      const pack = existing ?? generateContentPack(item.product);
      if (!existing) {
        packs.push(pack);
        db.contentPacks.push(pack);
      }
      pairs.push({ product: item.product, pack });
    }
...
export async function runEveningWorkflow(date = todayISO()) {
  return updateDb((db) => {
    ...
    const nextFocus = rankProducts(db.products, 3).map((r) => r.product.name);

    const recommendations = [
      ...analysis.recommendations,
      nextFocus.length
        ? `สินค้าแนะนำวันถัดไป (จากคะแนน): ${nextFocus.join(", ")}`
