...
export function captionForChannel(
  ...
  if (channel === "tiktok") {
    // voiceover already includes opening hook + disclosure — avoid duplicating both
    const tags = [...pack.hashtagsTh.slice(0, 3), ...pack.hashtagsEn.slice(0, 2)].join(
      " ",
    );
    return `${pack.tiktokScript.voiceover}\n\n${cta}\n\n${tags}`;
  }
...
export function buildDailySchedule(params: {
  ...
  // Cap total active posts/day (draft+approved+posted), not just this batch — prevents force-rerun spam
  const existingActiveToday = params.existing.filter(
    (s) => s.date === params.date && s.status !== "skipped",
  ).length;
  const room = Math.max(0, maxPosts - existingActiveToday);
  if (room === 0) return [];
  ...
  for (const pick of params.ranked) {
    ...
    const orderedSlots = [
      ...(preferLearnedTime
        ? slots.filter((s) => s.time === learning!.preferredTime)
        : []),
  ...
  // Prefer at least 2 posts when inventory allows — never exceed remaining daily room
