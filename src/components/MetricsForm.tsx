...
export function MetricsForm({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/schedule/${postId}/metrics`, {
      ...
      body: JSON.stringify({
        ...
        promoSpend: Number(form.get("promoSpend") ?? 0),
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="grid gap-2 rounded-xl border border-[var(--line)] bg-white/60 p-3 md:grid-cols-6"
    >
      <p className="md:col-span-6 text-sm font-medium text-[var(--sage-deep)]">
        กรอกผล: {productName}
      </p>
      ...
      <label className="text-xs">
        ...
        <input
          ...
          className={field}
          defaultValue={0}
          title="ads/boost, ตัวอย่างสินค้า, ค่าผลิต — ใช้คำนวณ ROI จริง"
        />
      </label>
      <label className="text-xs">
        โน้ต
      ...
      <div className="md:col-span-6 flex flex-wrap items-center gap-2">
        ...
        <span className="text-[10px] text-[var(--ink-soft)]">
          ROI = (ค่าคอม − ต้นทุน) ÷ ต้นทุน · ไม่กรอกต้นทุน = ดูได้แค่ค่าคอม/คลิก
        </span>
